...
def tile_y_coordinates(start_orthogonal_slope, end_orthogonal_slope, X_rect, X, Y, node):
    startY1 = calculate_tile_y_limits(start_orthogonal_slope, X_rect[0], X[node], Y[node])
    startY2 = calculate_tile_y_limits(start_orthogonal_slope, X_rect[1], X[node], Y[node])
    endY2 = calculate_tile_y_limits(end_orthogonal_slope, X_rect[2], X[node + 1], Y[node + 1])
    endY1 = calculate_tile_y_limits(end_orthogonal_slope, X_rect[3], X[node + 1], Y[node + 1])
