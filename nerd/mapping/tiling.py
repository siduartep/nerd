...
def slopes_from_coordinates(X,Y,node):
    start_slope = orthogonal_slope(slope_between_two_points(Y[node+1], Y[node-1], X[node+1], X[node-1]))
    end_slope = orthogonal_slope(slope_between_two_points(Y[node+2], Y[node], X[node+2], X[node]))
