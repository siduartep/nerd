function ansCoastline = setCoastlineDlg(ansCoastline)
dim = [15 3];
...
% static text
Prompt = cell(1,2);
Prompt{1,1} = ['Coasline files in Shapefile format'];
...
Prompt(end+1,:) = {'Coastline files','CoastlineFiles'};
Formats(2,1).type = 'edit';
Formats(2,1).format = 'file';
Formats(2,1).limits = [0 10]; % multi-select files
Formats(2,1).size = 800;
Formats(2,1).items = {'*.shp','Coastline Shapefile';'*.*','All Files'};
...
DefAns.CoastlineFiles = ansCoastline.CoastlineFiles;
[Answer,Cancelled] = inputsdlg(Prompt,Title,Formats,DefAns,Options);
ansCoastline.CoastlineFiles=Answer.CoastlineFiles;
